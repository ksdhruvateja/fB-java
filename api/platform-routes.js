...
export function registerPlatformRoutes(app, { pool, requireAuth, requireAdmin, pushStatus }) {
  ...
  app.get('/api/platform/plans', (_req, res) => {
    res.json({
      ok: true,
      plans: Object.entries(PLAN_CATALOG).map(([code, p]) => ({ code, ...p })),
    });
  });
    ...
    try {
      ...
      if (simulate) {
        const { rows } = await pool.query(
          ...
          [req.authUser.id, planCode, plan.family, JSON.stringify(plan)]
        );
      ...
      const checkout = await createCheckoutSession({
        ...
        description: `${brand.productName} ${plan.label}`,
        successPath: `/?paid=subscription&plan=${planCode}`,
        cancelPath: `/?canceled=subscription`,
        metadata: {
      ...
      await pool.query(
        ...
        [req.authUser.id, plan.amount, checkout.sessionId, JSON.stringify({ planCode })]
      );
    ...
    try {
      ...
      const { rows: refundRows } = await pool.query(
        ...
        [
          ...
          simulate,
          req.authUser.id,
          simulate ? `sim_re_${Date.now()}` : null,
        ]
      );
      ...
      await audit(pool, req.authUser.id, 'payment_refunded', 'payment', paymentId, { amount, reason, simulate });
      notifyOps(`Refund ${simulate ? '(sim) ' : ''}$${amount} on payment #${paymentId}`);
      res.json({ ok: true, refund: refundRows[0], simulated: simulate });
    } catch (e) {
      console.error(e);
      res.status(500).json({ ok: false, message: 'Could not refund.' });
    }
  });
