...
function normalizeDiyRiskLevel(value: unknown): "red" | "yellow" | "green" {
  const text = String(value ?? "green").trim().toLowerCase();
  if (["red", "high", "danger", "critical"].includes(text)) return "red";
  if (["yellow", "amber", "moderate", "caution"].includes(text)) return "yellow";
  return "green";
}
...
export default function HomeownerDashboard({
  ...
  useEffect(() => {
    ...
    const numericalUserId = Number(user.id);
    const numericalJobId = Number(activeJob.id);

    if (Number.isNaN(numericalUserId) || Number.isNaN(numericalJobId)) return;

    const savedChat = readDiyChat<ChatMessage>(numericalUserId, numericalJobId);
    setDiyChatMessages(
      savedChat?.length
        ? savedChat
        : [
  ...
  // 🔥 FIXED TYPESAFE BLOCK FOR WRITE LOGIC
  useEffect(() => {
    if (!user?.id || !activeJob?.id || !diyChatMessages.length) return;

    // Force both parameters into strict numerical data primitives
    const numericalUserId = Number(user.id);
    const numericalJobId = Number(activeJob.id);

    // Execute storage action only if both parameters successfully transformed into valid numbers
    if (!Number.isNaN(numericalUserId) && !Number.isNaN(numericalJobId)) {
      writeDiyChat(numericalUserId, numericalJobId, diyChatMessages);
    }
  }, [activeJob, user.id, diyChatMessages]);
  ...
  async function openDiySafetyModal(after: "guided" | "chat", pendingChat?: string) {
    if (getHomeownerDiyRisk(activeJob) === "red") {
  ...
  function persistDiyProgress(nextIndex: number, completed: Record<number, boolean>) {
    ...
    if (user.id == null || Number.isNaN(Number(user.id))) return;
    writeDiyProgress(Number(user.id), {
      jobId: activeJob.id,
          ...
          try {
            ...
            if (r.ok && r.job) {
              ...
              if (!r.job.aiAssessment && !isAssessmentProcessing(r.job)) {
                const zip = (r.job as any).zip || r.job.cityStateZip?.match(/\b\d{5}\b/)?.[0] || null;
  ...
  useEffect(() => {
    ...
    if (step === "assessment" && !activeJob && pendingServiceRequestId == null && !pendingAssessment && assessLoadingStep == null && !hireScreenOpen) {
      setStep("intake");
      setIntakePhase("describe");
    }
    if (step === "intake") {
  ...
  }, [tab, step, activeJob, pendingServiceRequestId, pendingAssessment, assessLoadingStep, hireScreenOpen]);
  ...
  async function runAssessWithProgress(
    ...
    if (
      !force &&
      assessInFlightRef.current === recordId
    ) {
      console.warn(
        "[Fixera] Assessment already in progress:",
        {
          recordId,
          recordType,
        }
    ...
    try {
      ...
      if (isPending) {
        console.log(
          "[Fixera] BEFORE assessPendingServiceRequest()",
          {
            pendingServiceRequestId: recordId,
            force,
            hasConsent:
              Boolean(aiAssessmentConsentRef.current),
          }
        );
        ...
        if (!result.ok) {
          console.error(
            "[Fixera] Pending assessment failed:",
            result
          );
        ...
        /*
         * Pending requests do not have a managed job yet.
         *
         * The assessment result belongs to:
         *
         * pending_service_requests.id
         *
         * so return the pending record rather than trying
         * to put it into activeJob.
         */
        return {
          ok: true as const,
          pendingServiceRequest:
            result.pendingServiceRequest,
          assessment:
            result.pendingServiceRequest?.aiAssessment,
          pricing:
            result.pricing ??
            result.pendingServiceRequest?.pricing,
          warning:
            result.warning ?? null,
      ...
      // ============================================================
      console.log(
        "[Fixera] BEFORE assessManagedJob()",
        {
          jobId: recordId,
          force,
          hasConsent:
  ...
  async function submitIssue(path: "ai" | "experts") {
    ...
    try {
      ...
      if (created.type === "pending_service_request") {
        const pendingId = Number(created.pendingServiceRequestId);

        if (!Number.isFinite(pendingId) || pendingId <= 0) {
          console.error("[Fixera] Invalid pending service request ID:", created.pendingServiceRequestId);
          setError("Your request was saved, but we could not start the assessment.");
          setHireScreenOpen(false);
          return;
        }
        ...
        setHireScreenOpen(false);
        setAssessmentMode(path === "experts" ? "expert" : "diy");
        setReportPath(path);
        ...
        console.log("[Fixera] Starting pending Fixera assessment:", pendingId);

        const assessed = await runAssessWithProgress(
          pendingId,
          propZip,
          { recordType: "pending_service_request" }
        );
        ...
        <div key={section.label || `top-${si}`} className="space-y-1">
          ...
          {section.items.map((item) => (
            <button
              ...
              className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm transition-colors ${tab === resolveNavTab(item.id) ||
      ...
      <div className="space-y-1">
        ...
        {FOOTER_NAV.map((item) => (
          <button
            ...
            className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm transition-colors ${tab === resolveNavTab(item.id) || (item.id === "settings" && tab === "profile")
          ...
          <main className="min-w-0 flex-1 px-4 py-6 pb-24 lg:px-8 lg:pb-6">
            ...
            {tab === "report" && (
              <section className="mx-auto max-w-3xl space-y-5">
                        ...
                        {SERVICE_TIMING_OPTIONS.map((opt) => {
                          ...
                          return (
                            <motion.button
                              ...
                              className={`group relative overflow-hidden rounded-xl border px-4 py-4 text-left transition ${selected
                          ...
                          {TIME_WINDOW_OPTIONS.map((opt, idx) => {
                            ...
                            return (
                              <motion.button
                                ...
                                aria-pressed={selected}
                                className={`relative rounded-lg px-2 py-3 text-center transition ${selected
                                  ? "bg-[#FF4D1C] text-white"
                          ...
                          {[
                            ...
                            return (
                              <button
                                ...
                                className={`rounded-lg px-3 py-2 text-xs font-medium transition ${selected
                                  ? "bg-[#FF4D1C] text-white"
                        ...
                        {PROPERTY_PURPOSE_OPTIONS.map((opt) => {
                          ...
                          return (
                            <motion.button
                              ...
                              aria-pressed={selected}
                              className={`flex items-center gap-2.5 rounded-xl border px-3 py-3 text-left text-sm transition ${selected
                            ...
                            >
                              <span
                                className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${selected ? "bg-[#FF4D1C] text-white" : "bg-muted text-[#FF4D1C]"
                                  }`}
                              >
                        ...
                        {PROJECT_STAGE_OPTIONS.map((opt, i) => {
                          ...
                          return (
                            <motion.button
                              ...
                              aria-pressed={selected}
                              className={`relative flex min-w-[9.5rem] shrink-0 flex-col gap-1 rounded-xl border px-3 py-3 text-left transition ${selected
                ...
                {step === "assessment" && (activeJob || pendingAssessment || pendingServiceRequestId != null || assessLoadingStep != null || hireScreenOpen) && (
                        ...
                        {assessmentStringList(pendingAssessment.pendingServiceRequest.aiAssessment.immediate_safety_steps).length > 0 ? (
                          <div className="rounded-md border border-amber-300 bg-amber-50 p-3 text-sm text-amber-950 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-100">
                            ...
                            <ul className="mt-1 list-disc pl-5">
                              {assessmentStringList(pendingAssessment.pendingServiceRequest.aiAssessment.immediate_safety_steps).map((s) => (
                                <li key={s}>{s}</li>
                              ))}
                            </ul>
                          </div>
                        ...
                        {assessmentMode === "diy" ? (
                          !hasDiyAccess ? (
                            <div className="relative space-y-4">
                              <div className="rounded-[1.5rem] bg-[#F3F4F6] p-4 dark:bg-muted/40 sm:p-6">
                                <div className="mb-4 text-center">
                                  <h3 className="[font-family:'Barlow_Condensed',sans-serif] text-2xl font-black uppercase tracking-tight">Unlock DIY Action Plan</h3>
                                  <p className="mt-1 text-xs text-muted-foreground">Upgrade to HomeCare Pro for full DIY action plans — includes a 7-day free trial.</p>
                                </div>
                                <HomeownerGoProPlans
                                  compact
                                  currentPlanCode={user.planCode}
                                  ...
                                  onPlansLoaded={(cards) => {
                                    setGoProPlans(cards);
                                    setDiyUnlockCodes(cards.filter((c) => c.unlocksDiy).map((c) => c.planCode));
                                  }}
            ...
            {tab === "jobs" && (
              <section className="mx-auto max-w-5xl space-y-4">
                ...
                {invoicePaymentMsg ? (
                  <p
                    className={`rounded-xl border px-4 py-3 text-sm ${invoicePaymentMsg.includes("OK")
                ...
                <div className="flex gap-1 overflow-x-auto rounded-xl border border-border bg-muted/40 p-1">
                  {JOBS_SEGMENTS.map((s) => (
                    <button
                      ...
                      className={`shrink-0 rounded-lg px-3 py-2 text-sm font-semibold transition ${jobsSegment === s.id ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"
                        }`}
                    >
                      {s.label}
                    </button>
                  ...
                  <div className="grid gap-4 lg:grid-cols-[0.9fr_1.2fr]">
                    <div className={`space-y-2 ${isMobile && selectedJobId ? "hidden" : ""}`}>
                      {filteredJobs.map((job) => (
                        <button
                          ...
                          className={`w-full rounded-2xl border p-3.5 text-left transition ${(selectedJobId ?? filteredJobs[0]?.id) === job.id
                    ...
                    {(selectedJob ?? filteredJobs[0] ?? null) && (
                      <div className={`space-y-3 ${isMobile && !selectedJobId ? "hidden" : ""}`}>
                        ...
                        <div id={`job-tracking-section-${(selectedJob ?? filteredJobs[0] ?? null)?.id || ""}`}>
                          <ServiceTrackingCard
